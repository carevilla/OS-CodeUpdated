...
//Add the following definitions near the top of kernel/proc.c:
...
freeproc(struct proc *p)
{
  ...
  // Hw 5 added
  //Task 1c.
  //Code to be added to freeproc() in proc.c (after p->trapframe = 0;):
...
userinit(void)
{
  ...
  //Add the following line to userinit() in proc.c:
  p->cur_max = MAXVA - (2*PGSIZE);
  //Also propagate cur_max to the child process in fork().
...
fork(void)
{
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  // b.   Add the following code after pid = np->pid; in fork() in kernel/proc.c:
...
//Add the following functions to kernel/proc.c and call mmrlistinit() in kernel/main.c:
